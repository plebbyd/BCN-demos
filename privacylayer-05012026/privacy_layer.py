...
def blur_regions(frame, boxes, method="box", strength=25, pad_frac=0.15):
    """Mask every box in-place on frame using the named method.

    Used for both face boxes and full person boxes -- the only difference
    is which detector produced the boxes.
    """
...
def blur_faces(frame, boxes, method="box", strength=25, pad_frac=0.15):
    """Backwards-compatible alias of blur_regions for face boxes."""
    return blur_regions(frame, boxes, method=method,
                        strength=strength, pad_frac=pad_frac)